...
io.on('connection', (socket) => {
  ...
  // 玩家加入
  socket.on('reportLocation', (coords) => {
    users[socket.id] = { id: socket.id, name: generateName(), lat: coords.lat, lng: coords.lng };
    io.emit('updateMap', users);
    socket.emit('yourNameIs', users[socket.id].name);
    io.emit('chatMessage', { name: '系統', text: `${users[socket.id].name} 已連線`, isSystem: true });
  });
    ...
    if (users[socket.id]) {
        ...
        if (urls && urls.length > 0) {
            try {
                const { result } = await ogs({ url: urls[0], timeout: 2000 });
                if (result.ogImage && result.ogImage.url) {
                    previewData = { title: result.ogTitle || "連結預覽", image: result.ogImage.url, url: urls[0] };
                }
            } catch (err) { console.log("預覽失敗"); }
        }
        const msgData = { id: socket.id, name: users[socket.id].name, text: msg, preview: previewData, isSystem: false, time: new Date().getTime() };
        messageHistory.push(msgData);
        if (messageHistory.length > MAX_HISTORY) messageHistory.shift();
        saveHistory();
        io.emit('chatMessage', msgData);
  ...
  // --- 3. 語音通話 ---
  socket.on('join-voice', (peerId) => { socket.broadcast.emit('user-connected-voice', peerId); });

  socket.on('disconnect', () => {
    if (users[socket.id]) {
        io.emit('chatMessage', { name: '系統', text: `${users[socket.id].name} 下線了`, isSystem: true });
        socket.broadcast.emit('user-disconnected-voice', socket.id);
